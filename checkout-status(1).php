...
<head>
    ...
    <!-- AUTO REDIRECT SCRIPT - PipraPay Automatic Redirect Feature -->
    <?php
    $status = strtolower($data['transaction']['status'] ?? 'pending');
    $returnUrl = $data['transaction']['return_url'] ?? '';
    
    // Only auto-redirect if payment is completed and return_url is valid
    if ($status === 'completed' && $returnUrl !== '' && $returnUrl !== '--'): 
    ?>
    <script>
        (function() {
            var redirectUrl = "<?php echo htmlspecialchars($returnUrl); ?>";
            var countdownTime = 4; // 4 seconds
            var countdownElement;
            var redirectTimer;
            var isRedirectCancelled = false;

            // Function to start countdown
            function startCountdown() {
                countdownElement = document.getElementById('redirect-countdown');
                if (countdownElement) {
                ...
                redirectTimer = setInterval(function() {
                    ...
                    if (countdownTime <= 0) {
                        clearInterval(redirectTimer);
                        if (!isRedirectCancelled && redirectUrl) {
            ...
            window.cancelAutoRedirect = function() {
                ...
                clearInterval(redirectTimer);
                var notice = document.getElementById('auto-redirect-notice');
                if (notice) {
    ...
    <div class="container container-tight py-4">
        <div class="card">
            <div class="card-body text-center">
                ...
                <!-- AUTO REDIRECT NOTICE - Only shows for completed payments -->
                <?php if ($status === 'completed' && $returnUrl !== '' && $returnUrl !== '--'): ?>
                <div id="auto-redirect-notice" class="redirect-notice">
                    <div>
                        <i class="bi bi-arrow-repeat spin"></i> 
                        Redirecting in <span id="redirect-countdown" class="countdown">4</span> seconds...
                    </div>
                    <div class="redirect-cancel">
                        <a onclick="cancelAutoRedirect(); return false;">Click here to stay on this page</a>
                    </div>
                </div>
                <?php endif; ?>
                <!-- END AUTO REDIRECT NOTICE -->
